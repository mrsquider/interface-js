...
onPageLoad(_ => {
  ...
  //listen for changes to see when we can change the QR-form.
  ...
  //observer code, just calls the modifyNativeQR function.
...
function showPostFormClassic(hide) {
  console.log("showPostFormClassic")
...
function showPostFormQR(hide) {
  console.log("showPostFormQR")
  var formSelector = "#qrForm"
  var nameField = query(formSelector + " input[name=name]")
  var optionsField = query(formSelector + " input[name=email]")
  var commentField = query(formSelector + " textarea")
  var interfaceSubmit = element(["button", {
    name: "Green Post",
    value: "Green Post",
  }])
  console.log(interfaceSubmit);
  if (hide) {
    if (postFormQR) {
      ...
      // hide the real QR post button
      unsafeWindow.QR.onClick = null;
      postFormQR = 0
    }
    return
  }
  ...
  postFormQR = element(
    ["form#form", {
      ...
      ["div", ["input", {
        ...
        value: "Post",
        onclick: event => {
          unsafeWindow.QR.submit = function() {
            return
          } // clear out the submit function to not see erroneous errors in the QR
...
`.replace(/\n\s*/g, "")
element(
  document.head || document.documentElement, ["style", {
    id: "s4sinterface-css"
  }, stylesheet]
)
